...
/* eslint-disable */
export class Init1690291642000 implements MigrationInterface {
  name = 'Init1690291642000';
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TABLE "calibration" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "createdAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "updatedAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "deletedAt" datetime, "volume" float DEFAULT (0), "level" float DEFAULT (0), "tankId" integer)`,
    );
    await queryRunner.query(
      `CREATE TABLE "tank_history" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "createdAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "updatedAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "deletedAt" datetime, "temperature" float NOT NULL DEFAULT (0), "volume" float NOT NULL DEFAULT (0), "weight" float NOT NULL DEFAULT (0), "density" float NOT NULL DEFAULT (0), "fuelId" integer, "fuelHolderId" integer, "refineryId" integer, "tankId" integer)`,
    );
    await queryRunner.query(
      `CREATE TABLE "tank" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "createdAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "updatedAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "deletedAt" datetime, "sortIndex" integer NOT NULL, "totalVolume" float NOT NULL, "deathBalance" float NOT NULL DEFAULT (0), "temperature" float NOT NULL DEFAULT (0), "volume" float NOT NULL DEFAULT (0), "weight" float NOT NULL DEFAULT (0), "density" float NOT NULL DEFAULT (0), "level" integer NOT NULL DEFAULT (0), "isEnabled" boolean NOT NULL DEFAULT (1), "isBlocked" boolean NOT NULL DEFAULT (0), "addressId" integer, "error" varchar, "fuelId" integer, "fuelHolderId" integer, "refineryId" integer)`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TABLE "temporary_calibration" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "createdAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "updatedAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "deletedAt" datetime, "volume" float DEFAULT (0), "level" float DEFAULT (0), "tankId" integer, CONSTRAINT "FK_4bca2e9e02afdb887385d082d00" FOREIGN KEY ("tankId") REFERENCES "tank" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION)`,
    );
    await queryRunner.query(
      `INSERT INTO "temporary_calibration"("id", "createdAt", "updatedAt", "deletedAt", "volume", "level", "tankId") SELECT "id", "createdAt", "updatedAt", "deletedAt", "volume", "level", "tankId" FROM "calibration"`,
    );
    await queryRunner.query(`DROP TABLE "calibration"`);
    await queryRunner.query(
      `ALTER TABLE "temporary_calibration" RENAME TO "calibration"`,
    );
    await queryRunner.query(
      `CREATE TABLE "temporary_tank_history" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "createdAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "updatedAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "deletedAt" datetime, "temperature" float NOT NULL DEFAULT (0), "volume" float NOT NULL DEFAULT (0), "weight" float NOT NULL DEFAULT (0), "density" float NOT NULL DEFAULT (0), "fuelId" integer, "fuelHolderId" integer, "refineryId" integer, "tankId" integer, CONSTRAINT "FK_63c1eea6329fd9152795369b6b4" FOREIGN KEY ("fuelId") REFERENCES "fuel" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT "FK_4c4e6072bcd6b29afb58e19430a" FOREIGN KEY ("fuelHolderId") REFERENCES "fuel_holder" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT "FK_a1f1da773d9b3f7c0d7e85c280b" FOREIGN KEY ("refineryId") REFERENCES "refinery" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION, CONSTRAINT "FK_39a56465e3f2048e0fc62fca002" FOREIGN KEY ("tankId") REFERENCES "tank" ("id") ON DELETE NO ACTION ON UPDATE NO ACTION)`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `INSERT INTO "temporary_tank"("id", "createdAt", "updatedAt", "deletedAt", "sortIndex", "totalVolume", "deathBalance", "temperature", "volume", "weight", "density", "level", "isEnabled", "isBlocked", "addressId", "error", "fuelId", "fuelHolderId", "refineryId") SELECT "id", "createdAt", "updatedAt", "deletedAt", "sortIndex", "totalVolume", "deathBalance", "temperature", "volume", "weight", "density", "level", "isEnabled", "isBlocked", "addressId", "error", "fuelId", "fuelHolderId", "refineryId" FROM "tank"`,
    );
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `INSERT INTO "tank"("id", "createdAt", "updatedAt", "deletedAt", "sortIndex", "totalVolume", "deathBalance", "temperature", "volume", "weight", "density", "level", "isEnabled", "isBlocked", "addressId", "error", "fuelId", "fuelHolderId", "refineryId") SELECT "id", "createdAt", "updatedAt", "deletedAt", "sortIndex", "totalVolume", "deathBalance", "temperature", "volume", "weight", "density", "level", "isEnabled", "isBlocked", "addressId", "error", "fuelId", "fuelHolderId", "refineryId" FROM "temporary_tank"`,
    );
    await queryRunner.query(`DROP TABLE "temporary_tank"`);
    await queryRunner.query(
      `ALTER TABLE "tank_history" RENAME TO "temporary_tank_history"`,
    );
    await queryRunner.query(
      `CREATE TABLE "tank_history" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "createdAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "updatedAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "deletedAt" datetime, "temperature" float NOT NULL DEFAULT (0), "volume" float NOT NULL DEFAULT (0), "weight" float NOT NULL DEFAULT (0), "density" float NOT NULL DEFAULT (0), "fuelId" integer, "fuelHolderId" integer, "refineryId" integer, "tankId" integer)`,
    );
    await queryRunner.query(
      `INSERT INTO "tank_history"("id", "createdAt", "updatedAt", "deletedAt", "temperature", "volume", "weight", "density", "fuelId", "fuelHolderId", "refineryId", "tankId") SELECT "id", "createdAt", "updatedAt", "deletedAt", "temperature", "volume", "weight", "density", "fuelId", "fuelHolderId", "refineryId", "tankId" FROM "temporary_tank_history"`,
    );
    await queryRunner.query(`DROP TABLE "temporary_tank_history"`);
    await queryRunner.query(
      `ALTER TABLE "calibration" RENAME TO "temporary_calibration"`,
    );
    await queryRunner.query(
      `CREATE TABLE "calibration" ("id" integer PRIMARY KEY AUTOINCREMENT NOT NULL, "createdAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "updatedAt" integer NOT NULL DEFAULT (strftime('%s', 'now')), "deletedAt" datetime, "volume" float DEFAULT (0), "level" float DEFAULT (0), "tankId" integer)`,
    );
    await queryRunner.query(
      `INSERT INTO "calibration"("id", "createdAt", "updatedAt", "deletedAt", "volume", "level", "tankId") SELECT "id", "createdAt", "updatedAt", "deletedAt", "volume", "level", "tankId" FROM "temporary_calibration"`,
    );
    await queryRunner.query(`DROP TABLE "temporary_calibration"`);
    await queryRunner.query(
