...
export class DispenserQueueService extends CommonService<DispenserQueue> {
  ...
  async checkState(payload: GetQueStateDto): Promise<DispenserRvResponseDto> {
    const dispenser = await this.dispenserService.findOne({
      where: {
        id: payload.idTrk,
      },
    });
    ...
    let result: DispenserRvResponseDto = {
      idTrk: dispenser.id,
